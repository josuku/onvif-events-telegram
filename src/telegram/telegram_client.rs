            ...
            let snapshot = match camera.client.get_snapshot().await {
                ...
                Err(err) => {
                    let error = format!(
                        ...
                        camera.client.snapshot_uri, err
                    );
                    error!("{}", error);
                    let _ = bot.send_message(msg.chat.id, error).await;
                    return Ok(()); // TODO return error
                    ...
                    if !users.contains(&DEFAULT_USERNAME.to_string()) {
                        match create_default_user(&camera.client.uri).await {
                            Ok(_) => {
                                ...
                                // if let Some(ref snapshot_uri) = camera.client.snapshot_uri {
                                //     let fixed_snapshot_uri = replace_snapshot_uri_credentials(
                                //         &snapshot_uri,
                                //         DEFAULT_USERNAME,
                                //         DEFAULT_PASSWORD,
                                //     );
                                //     camera.client.fix_snapshot_uri(fixed_snapshot_uri.clone());
                                //     message =
                                //         format!("camera snapshot uri fixed:{}", fixed_snapshot_uri);
                                //     println!("{}", message);
                                //     let _ = bot.send_message(msg.chat.id, message).await;
                                // }
                                fix_snapshot_uri(&mut camera, &bot, msg.chat.id).await;
                            }
                            Err(err) => {
...
pub async fn fix_snapshot_uri(camera: &mut Camera, bot: &Bot, chat_id: ChatId) {
    if let Some(ref snapshot_uri) = camera.client.snapshot_uri {
        let fixed_snapshot_uri = replace_snapshot_uri_credentials(
            &snapshot_uri,
            DEFAULT_USERNAME,
