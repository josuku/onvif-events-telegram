...
impl OnvifCamera {
    ...
    pub async fn get_snapshot(&self) -> anyhow::Result<Vec<u8>> {
        if let Some(uri) = &self.snapshot_uri {
            println!("snapshot uri:{}", uri); // TODO DELETE
            ...
            if let Some(media) = &self.clients.media {
                match get_snapshot_uris(media).await {
                    Ok(uris) => {
                        ...
                        // done as explicit command apart
                    }
    ...
    pub fn fix_snapshot_uri(&mut self, new_snapshot_uri: String) {
        self.snapshot_uri = Some(new_snapshot_uri);
    }
}
