use crate::{config::CameraConfig, onvif::onvif_camera::OnvifCamera, CameraId};
use anyhow::bail;
use log::error;
...
impl MemoryRepository {
    ...
    pub async fn init(&self) -> anyhow::Result<()> {
        ...
        for camera in cameras {
            let mut client = OnvifCamera::new(CameraConfig {
                ip: camera.ip,
    ...
    pub async fn update_cameras(&self, new_devices: &[Device]) -> anyhow::Result<()> {
        for new_device in new_devices {
            ...
            {
                // TODO get snapshot uri and extract ip?, username and password, and snapshot_uri
                // TODO create new onvif specific user?
                ...
                let mut client = OnvifCamera::new(CameraConfig {
                    ip,
                    username: "josuku".to_string(), // TODO
                    password: "josuku".to_string(), // TODO
                    snapshot_uri: "http://192.168.1.217:8899/webcapture.jpg?command=snap&channel=0&user=josuku&password=josuku".to_string(), // TODO
                });
                client.init().await;
